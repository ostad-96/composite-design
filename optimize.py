...
from utils.trainer import Trainer # Trainer definition is in utils/trainer.py
from utils.dqn import DQNAgent, CompositeDesignEnv # DQNAgent and Env are in utils/dqn.py
from config import (
    MATRIX_SIZE, FCN_INPUT_CHANNELS,
    FCN_NUM_FILTERS_RESBLOCK, FCN_NUM_RES_BLOCKS as DEFAULT_FCN_BLOCKS,
    FCN_KERNEL_SIZE as DEFAULT_FCN_KERNEL_SIZE, FCN_DILATION_STRATEGIES,
    FCN_DILATION_PROGRESSIVE_MAX_BLOCKS, FCN_DILATION_PROGRESSIVE_PATTERN,
    FCN_DILATION_CYCLIC_PATTERN, REPLAY_BUFFER_CAPACITY, CLIP_GRAD_NORM_MAX,
    EPISODES_PER_CYCLE, OPT_STEPS_PER_CYCLE, NUM_CYCLES, TOTAL_TARGET_EPISODES
)

# --- Global Paths (Set by args later) ---
CHECKPOINT_DIR_BASE = "checkpoints_fcn_dil_loss" # Changed suffix for new runs
REPLAY_BUFFER_DIR_BASE = "replay_buffer_data_fcn_dil_loss" # Changed suffix
PLOT_DIR_BASE = "trial_plots_fcn_dil_loss" # Changed suffix
RESULTS_FILE_BASE = "trial_results_fcn_dil_loss.jsonl" # Changed suffix
...
def objective(trial: optuna.Trial) -> float:
    ...
    print(f"\n--- Starting Optuna Trial {trial_number} (FCN+Dil+Success+LossChoice) | Device: {device} ---")
    ...
    if resumable_checkpoint_path:
        ...
        if loaded_checkpoint_content:
            ...
            if loaded_agent_checkpoint_state and \
               ...
               'episode_success_flags' in trainer_chkpt_state : # Assuming agent HPs in chkpt have loss_fn
                ...
                if resumable_checkpoint_path != current_trial_checkpoint_path:
                    try:
                        ...
                        is_resuming = False; start_cycle = 0; total_episodes_offset = 0; total_steps_offset = 0; initial_rewards = []; initial_success_flags = []
    ...
    if is_resuming and hyperparams_to_use:
        ...
        is_resuming = False # Ensure this is false
        ...
        hyperparams_to_use = {
            ...
            "fcn_input_channels": FCN_INPUT_CHANNELS, # Fixed
            ...
            "loss_function": trial.suggest_categorical("loss_function", ["smooth_l1", "mse"]) # <<< Added suggestion
        }
    ...
    try:
        agent = DQNAgent(
            ...
            buffer_dir=REPLAY_BUFFER_DIR_BASE, # Use the global base for this trial
            clip_grad_norm_max=hyperparams_to_use['clip_grad_norm_max'],
            loss_function_name=hyperparams_to_use['loss_function'] # <<< Pass chosen loss function
        )
    except Exception as e:
        print(f"ERROR: DQNAgent init failed: {e}"); traceback.print_exc(); raise optuna.TrialPruned(f"Agent init error: {e}")
    ...
    if is_resuming and loaded_agent_checkpoint_state:
        ...
        try:
            ...
            print(f"ERROR loading agent state: {e}"); traceback.print_exc(); raise optuna.TrialPruned(f"Agent state load error: {e}")
    ...
    trainer = Trainer(
        ...
        checkpoint_dir=CHECKPOINT_DIR_BASE, # Trainer saves to its specific trial file
    ...
    try:
        ...
        if os.path.exists(current_trial_checkpoint_path):
            print(f" Checkpoint for interrupted T{trial_number} remains: {current_trial_checkpoint_path}")
             ...
             print(f" No specific checkpoint file found for currently interrupted T{trial_number} to preserve.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Optuna FCN DQN with Dilation, Success, Loss Choice")
    parser.add_argument("--trials", type=int, default=30, help="Number of Optuna trials to run.")
    parser.add_argument("--study-name", type=str, default="DQN_FCN_Dil_LossChoice", help="Name for the Optuna study.") # Updated study name
    parser.add_argument("--storage", type=str, default="sqlite:///dqn_fcn_dil_losschoice.db", help="Optuna storage URL.") # Updated db name
    ...
    RESULTS_FILE_BASE = args.results_file # This is now set by arg
    TOTAL_TARGET_EPISODES = args.target_episodes

    os.makedirs(CHECKPOINT_DIR_BASE, exist_ok=True)
    os.makedirs(REPLAY_BUFFER_DIR_BASE, exist_ok=True)
    ...
    print("\n--- Initializing Optuna Study (FCN Dilation+Success+LossChoice) ---")
    ...
    try:
        ...
        running_study_trials = [t for t in all_trials_from_study if t.state == optuna.trial.TrialState.RUNNING]
        waiting_study_trials = [t for t in all_trials_from_study if t.state == optuna.trial.TrialState.WAITING]
        ...
        if running_study_trials: print(f" Running: {len(running_study_trials)}")
        if waiting_study_trials: print(f" Waiting: {len(waiting_study_trials)}")

        best_optuna_trial = None
        try: best_optuna_trial = study.best_trial
        except ValueError: print(" No best trial found (e.g., no trials completed).")

        if best_optuna_trial:
            print(f"Best Optuna Trial (from study.best_trial):")
            ...
            if os.path.exists(REPLAY_BUFFER_DIR_BASE) and any(os.scandir(REPLAY_BUFFER_DIR_BASE)):
                ...
                if confirm == 'y':
                    try:
                        ...
                        os.makedirs(REPLAY_BUFFER_DIR_BASE) # Recreate
                        print(f" Buffer directory '{REPLAY_BUFFER_DIR_BASE}' cleared and recreated.")
                    except Exception as e_del: print(f" ERROR deleting buffer directory: {e_del}")
                else: print(" Buffer directory not deleted.")
            else: print(" Buffer directory is empty or does not exist. No cleanup needed.")
        print("\n--- Script Finished ---")